class Person {
    ...
    readonly id: number
    public name: string
    age: number // mặc định public
    private password: string // chỉ được sử dụng bên trong class
    protected isLove: boolean // chỉ được sử dụng bên trong class hoặc sử dụng bên trong class kế thừa
    // hàm khởi tạo
    constructor(id: number, name: string,age: number, password: string, isLove: boolean){
        this.id = id;
        this.name = name;
        this.age = age;
...
class Member extends Person{
    ...
    constructor(id: number, name: string,age: number, password: string, isLove: boolean,role: number){
